...
class SGD:
    """
        Matrix factorization with gradient descent. The dataset is expected in format <user, item, rating>.
    """
    ...
    def predict_dataset(self, trainset):
        """Make predictions for given dataset using current U, V matrices and biases."""
        return np.array([self.global_mean + self.bu[u] + self.bi[i] + np.dot(self.qi[i], self.pu[u])
                         for u, i, r in trainset])

    def predict_matrix(self):
        """Predict ratings with current U, V matrices and biases. Calculates full matrix n_users x n_items."""
